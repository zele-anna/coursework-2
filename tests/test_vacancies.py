...
def test_cast_to_object_list(hh_data: list) -> None:
    ...
    assert vacancies_list[0].requirement == "React TypeScript yoki Next.js TypeScript bo‘yicha bilim."
...
def test_object_to_dict(vacancy_object: Any, vacancy_dict: dict) -> None:
    vacancy_obj_to_dict = vacancy_object.object_to_dict()
    assert vacancy_obj_to_dict == vacancy_dict


def test_get_salary_data_from_dict() -> None:
    salary_from, salary_to, salary_range = Vacancy._Vacancy__get_salary_data_from_dict(
...
def test_get_salary_data_from_str() -> None:
    """Тест на получение данных из файла"""
    # salary_from, salary_to, salary_range = Vacancy.get_salary_data_from_str("2000000-10000000 UZS")
    salary_from, salary_to, salary_range = Vacancy._Vacancy__get_salary_data_from_str("2000000-10000000 UZS")
    assert salary_from == 2000000
    assert salary_to == 10000000
    assert salary_range == "2000000-10000000 UZS"
    salary_from, salary_to, salary_range = Vacancy._Vacancy__get_salary_data_from_str("От 2000000 UZS")
    assert salary_from == 2000000
    assert salary_to == 0
    assert salary_range == "От 2000000 UZS"
    salary_from, salary_to, salary_range = Vacancy._Vacancy__get_salary_data_from_str("До 10000000 UZS")
    assert salary_from == 0
    assert salary_to == 10000000
    assert salary_range == "До 10000000 UZS"


def test_vacancies_compare(capsys: Any, vacancy_object: Any, vacancy_object_2: Any) -> None:
    assert (vacancy_object > vacancy_object_2) is True
    assert (vacancy_object >= vacancy_object_2) is True
    assert (vacancy_object < vacancy_object_2) is False
    assert (vacancy_object <= vacancy_object_2) is False
    assert (vacancy_object == vacancy_object_2) is False
    type(vacancy_object > 1000)
    message = capsys.readouterr()
    assert message.out == "Объект сравнения не является объектом класса Vacancy.\n"
    type(vacancy_object >= 1000)
    message = capsys.readouterr()
    assert message.out == "Объект сравнения не является объектом класса Vacancy.\n"
    type(vacancy_object == 1000)
